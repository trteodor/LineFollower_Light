...
/*
 * This file has been modified by Teodor.R - adapt to STM32H743VI
 * Add some functionalities and includes for easier usage
 * */
...
uint16_t VirtAddVarTab[EE_VarsCount] = {0};
...
uint16_t EE_Init(void)
{
  ...
  /*Fill the virutal table addr data table based on type EE_Vars_t*/
      ...
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        for (VarIdx = 0; VarIdx < NB_OF_VAR; VarIdx++)
        {
          ...
          if (VarIdx != x)
          {
            ...
            ReadStatus = EE_ReadVariableU32(VirtAddVarTab[VarIdx], &DataVar);
...
{
  static volatile uint16_t ValidPage = PAGE0;
  static volatile uint16_t AddressValue = 0x5555, ReadStatus = 1;
  static volatile uint32_t Address = EEPROM_START_ADDRESS;
  static volatile uint32_t PageStartAddress = EEPROM_START_ADDRESS;
...
uint16_t EE_ReadVariableF32(EE_Vars_t VirtAddress, float* Data)
{
  ...
  uint16_t ReadStatus;
  uint32_t HelperVar32;  

  ReadStatus = EE_ReadVariableU32(VirtAddress,&HelperVar32);

  memcpy(Data,&HelperVar32,4);
